...
public final class GameStateScore implements FinalMoveSelectionStrategy {
    @Override
    public Move selectMove(MCTS mcts, BaseNode rootNode) {
        ...
        for (int i = 0; i < numChildren; ++i)
        {
            ...
            // Replace score by context score for SameGame
            final double childValueEstimate;
            if (mcts instanceof SameGameMCTS) {
                SameGameMCTS sameGameMCTS = (SameGameMCTS) mcts;
                childValueEstimate = child == null ? 0.0 : sameGameMCTS.normalizeScore(rootNode.contextRef());
            }
            else childValueEstimate = child == null ? 0.0 : child.expectedScore(moverAgent);
