...
public class Main {
        ...
        try {
            ...
            for(int i = 0; i < boardSizesList.size(); i++){
                for(int j = 0; j < colorsList.size(); j++){
                    ...
                    generateGames(pm, tx, numGames, boardSizesList.get(i), colorsList.get(j), reflectionTime);
    ...
    private static void generateGames(PersistenceManager pm, Transaction tx, int numGames, int boardSize, int numColours, int reflectionTime) {
...
//                tx.begin();
                ...
                StandardNode root = gameTrialData.rootNode();
